...
Arv *abb_retira(Arv *r, int valor) {
  if (r == NULL)
    return NULL;
  else if (r->info > valor)
    r->esq = abb_retira(r->esq, valor);
  else if (r->info < valor)
    r->dir = abb_retira(r->dir, valor);
  ...
  int fb = getBalanceamento(r);
  printf(">>>>> No: %d - Fb: %d\n", r->info, fb);

  // Rotação Simples
  ////Direita
  if (fb > 1 && getBalanceamento(r->esq) >=0)
    r = rotDir(r);
  ////Esquerda
  if (fb < -1 &&  getBalanceamento(r->dir) <= 0)
    r = rotEsq(r);
...
int main() {
  ...
  // Preparação para desbalancear na remoção
  // Ajustar o código baseado em:
  ////// https://www.geeksforgeeks.org/deletion-in-an-avl-tree/
  root = insereAVL(root, 5);
  printf("\n>>>>>>>>>>>>>>>>>\nABB Atual\n");
  print2DTree(root, 0, 5);
  root = abb_retira(root, 50);
  printf("\n>>>>>>>>>>>>>>>>>\nABB Atual\n");
  print2DTree(root, 0, 5);
}
